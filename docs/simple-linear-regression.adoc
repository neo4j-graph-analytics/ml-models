...
Visually, there is a plot of several (x, y) data points. Simple linear regression finds the line through the points that _best "fits" the data_. This is one of the most popular tools in statistics, and it is frequently used as a predictor for machine learning.
// end::introduction[]

== Explanation and history

// tag::explanation[]
At the core of linear regression is the method of least squares. In this method, we find the coefficients *a* and *b* that minimize the sum of squared differences between each observation's *actual* y-value and its *predicted* y-value according to the linear model `y = a + b*x`. 
